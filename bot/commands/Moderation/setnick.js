...
const nickSchema = require('../../schemas/nicknames')
...
module.exports = {
    ...
    run: async (message, args, text, client, prefix, instance) => {
        ...
        if (target) {
            ...
            if (newName.length > 32) return message.reply("New nickname cannot be longer than 32 characters")
            if (newName.length < 2) return message.reply("New nickname cannot be longer than 2 characters")

            if (staff.roles.highest.position < target.roles.highest.position) return message.reply(`You cannot change ${targetTag}'s nickname due to role hierarchy.`)
            if (target === message.guild.owner) return message.reply(`You/I cannot change ${targetTag}'s nickname because they are the owner of the server.`)
            ...
            await mongo().then(async (mongoose) => {
                try {
                    ...
                    const success = new Discord.MessageEmbed()
                        ...
                        .setDescription(`Successfully changed **${targetTag}'s** nickname to **${newName}**.`)
            ...
            if (target === message.guild.owner) return message.reply(`I cannot change ${targetTag}'s nickname because they are the owner of the server.`)
