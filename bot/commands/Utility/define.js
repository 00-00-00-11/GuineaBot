...
module.exports = {
    ...
    run: async ({
        ...
        axios.get(`https://dictionaryapi.com/api/v3/references/thesaurus/json/${word}?key=${process.env.DICTIONARY1}`).then(async (data, err) => {
            ...
            let synURL = await bin.create([{
                name: "GuineaBot/Cy1der",
                content: `${alldata[0].meta.syns.length > 1 ? alldata[0].meta.syns : "none"}`,
                languageId: "JSON"
            }], {
            ...
            let antURL = await bin.create([{
                name: "GuineaBot/Cy1der",
                content: `${alldata[0].meta.ants.length > 1 ? alldata[0].meta.ants : "none"}`,
                languageId: "JSON"
            }], {
                title: `Synonyms for ${alldata[0].meta.id}`,
                description: "Dictionary command executed"
            ...
            let embed = new Discord.MessageEmbed()
                ...
                .setDescription(`**Offensive?** ${alldata[0].meta.offensive ? "Yes" : "No"}`)
                .addFields({
                    name: "Definition(s)",
