...
module.exports = {
    ...
    description: 'Fully customizable process to create a VOICE channel',
    run: async (message, args, client) => {
        ...
        message.channel.send("What will the bitrate be? Enter a number between `8` and `96` (default is 64).\n`Note, anything higher than 64 can be unstable for people with a poor internet connection!`")
        ...
        if (isNaN(bitrate)) {
            message.channel.send('Oh no! It looks like you did not respond with number. Please redo this procedure from the beginning.')
            return
        }

        if (bitrate < 8 || bitrate > 96) {
            message.channel.send('Oh no! It looks like you responded with an invalid number. Please redo this procedure from the beginning.')
            return
        }
        ...
        await message.channel.send("Ok, which category should I create the channel in? `Note: case sensitive`\nIf you do not wish to create a voice channel in a category, please respond with a `none`")
        ...
        if (answer3 && answer3 !== "none") {
            cat = message.guild.channels.cache.find(cat => cat.name === `${answer3}` && cat.type === 'category')
        ...
        let settingsset = JSON.stringify(settings, null, 4)
