...
module.exports= {
    ...
    run: async(message, args, client) => {
        ...
        if(!message.member.hasPermission("MANAGE_MESSAGES", explicit = true)) {
            ...
            message.channel.bulkDelete(msgdel, {filterOld: true}).then(delsize => {
                message.channel.send(`Deleted ${delsize.size} messages. \`This message will self destruct in 3 seconds\``).then(msg => {
                    msg.delete({timeout: 3000})
                })
                if (msgdel !== delsize.size) {
                    message.channel.send("**Due to limitations, if I did not delete as many messages as you specified it's because I am unable to delete messages older than 14 days.**\n\`This message will self destruct in 5 seconds\`").then(message1 => {
                        message1.delete({timeout: 5000})
                    })
                }
                console.log(delsize.size)
                console.log(msgdel)
            })
        }
    }
