...
module.exports= {
    ...
    run: async(message, args, client) => {
        ...
        message.channel.send('**Starting text channel creation process... Enter `cancel` anytime to end the process.**')
        message.channel.send('What is the topic of the channel? If you do not wish to have one, please enter `none`.')
        ...
        if (answer === 'cancel' || answer === 'Cancel') {
            message.channel.send(`**Setup process ended, requested by ${message.author.tag}.**`)
            return;
        }
        ...
        await message.channel.send('Great! Which category should I create the channel in `Note: case sensitive`? If you do not wish to create a channel in a category, please enter `none`.')
        ...
        if (answer2 && answer2 !== "none") {
            cat = message.guild.channels.cache.find(cat => cat.name === `${answer2}` && cat.type === 'category')
            settings.parent = cat.id
        }

        if (cat === undefined) {
            message.channel.send('Oh no! It looks like you did not respond with a valid category or I could not find it. Please redo this procedure from the beginning.')
            return
        }

        await message.channel.send('Awesome! Is the channel going to be an NSFW channel? Respond with a `yes` or a `no`')
        ...
        if (answer3 === "yes" || answer3 === "Yes") {
            ...
            message.channel.send('Oh no! It looks like you did not respond with a `yes` or `no`. Please redo this procedure from the beginning.')
            return
        }

        await message.channel.send('Next, will this channel have a slowmode?\nIf yes, please enter one of the following: `5` `10` `15` `30` `60` `120` `300` `600` `900` `1800` `3600` `7200` `21600` *values are in seconds*\nIf no, please enter `none` or `0`')
        ...
        /*
        if (answer4 !== "5" || answer4 !== "10" || answer4 !== "15" || answer4 !== "30" || answer4 !== "60" || answer4 !== "120" || answer4 !== "300" || answer4 !== "600" || answer4 !== "900" || answer4 !== "1800" || answer4 !== "3600" || answer4 !== "7200" || answer4 !== "21600") {
            message.channel.send('Oh no! It looks like you did not respond with a valid answer. Please redo this procedure from the beginning.')
            return
        }
        */
        ...
        await message.channel.send('What position will the channel be created? The higher the number, the higher up it will be created. *the highest number possible is `9007199254740991` but I don\'t see why you need that many channels lmao*')
        ...
        settings.position = positionW

        if (isNaN(position)) {
            message.channel.send('Oh no! It looks like you did not respond with a number. Please redo this procedure from the beginning.')
            return
        }

        await message.channel.send("Ok, what is the minimum role that can access this channel `Note: case sensitive and do NOT mention the role`? If you do not wish to have a role, please enter with a `none`.")
        ...
        if (answer6 === "none" || answer6 === "None") {
            ...
            if (minrole === undefined) {
                message.channel.send('Oh no! It looks like you did not respond with a valid role or I was unable to find it. Please redo this procedure from the beginning.')
                return
            }
        ...
        await message.channel.send("Almost done! Is there a reason for creating the channel? If not, respond with a `no`.")
        ...
        await message.channel.send("Finally, what is the name of the channel? Remember, spaces will become \"-\" and all uppercase letters will be lowercase.")
        ...
        .addFields(
            ...
            { name: "Channel settings: ", value: `${channelset}`},
