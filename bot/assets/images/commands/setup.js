...
module.exports= {
    name: 'setup',
    category: '.mandatory',
    description: 'All the required values the bot needs',
    run: async(message, args, client) => {
        ...
        if (!minrole) {
            message.channel.send('**Oops! I could not find the role. Perhaps try doing the command again.**')
            return
        }
        ...
        if (!category) {
            message.channel.send('**Oops! I could not find the category. Perhaps try doing the command again.**')
            return
        }
        ...
        message.guild.roles.create({
            data: {
                name: 'gmuted',
                color: '#800000',
                permissions: ["CREATE_INSTANT_INVITE", "VIEW_CHANNEL", "READ_MESSAGE_HISTORY"],
                mentionable: true,
        ...
        if (readme) {
            ...
            message.guild.channels.create('read-me', {type: 'text'})
