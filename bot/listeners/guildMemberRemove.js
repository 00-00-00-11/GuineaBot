...
module.exports = (client) => {
    client.on("guildMemberRemove", async member => {
        let channel = member.guild.channels.cache.find(channel => channel.name.includes("g-goodbye"))
        if (!channel) return
        ...
        const goodbyeCard = await goodbyeCanvas
            ...
            .setBackground("https://cdn.trendhunterstatic.com/thumbs/cool-backgrounds.jpeg")
            .toAttachment()

        const attachment = new Discord.MessageAttachment(goodbyeCard.toBuffer(), "goodbye.png")
        channel.send(attachment)

        let modlog = member.guild.channels.cache.find(channel => channel.name.includes("g-modlog"))
        if (!modlog) return
